...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, defaultTab = 'login' }) => {
  ...
  const [adminBypassPhone, setAdminBypassPhone] = useState('');
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (!phone) {
      setErrorMsg(loginRole === 'Farmer' ? 'কৃষকের নাম বা মোবাইল নম্বর প্রদান করুন।' : 'মোবাইল নম্বর প্রদান করুন।');
      return;
    }

    const res = login(phone, loginRole, password, adminBypassPhone);
    if (res.success) {
        ...
        <div className="p-6">
          ...
          {activeTab === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              {/* Mobile phone or Farmer name input */}
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1.5 font-sans">
                  {loginRole === 'Farmer' ? 'খামারির নাম বা মোবাইল নম্বর (Name or Mobile)' : 'মোবাইল নম্বর (Mobile Number)'}
                </label>
                <div className="relative">
                  ...
                  <input
                    type="text"
                    required
                    placeholder={loginRole === 'Farmer' ? 'যেমন: Karim Mia বা ফোন নম্বর' : '১১ ডিজিটের মোবাইল নম্বর দিন'}
                    value={phone}
                    onChange={(e) => {
              ...
              {/* Farmer role special bypass field */}
              {loginRole === 'Farmer' && (
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1.5 font-sans">
                    অ্যাডমিন মোবাইল নম্বর (Admin Mobile Override - ঐচ্ছিক)
                  </label>
                  <div className="relative">
                    <Phone className="absolute left-3.5 top-3 h-4 w-4 text-amber-500" />
                    <input
                      type="tel"
                      placeholder="অ্যাডমিন নম্বর দিন (যদি ওভাররাইড করতে চান)"
                      value={adminBypassPhone}
                      onChange={(e) => setAdminBypassPhone(e.target.value)}
                      className="w-full rounded-2xl border border-dashed border-amber-300 bg-amber-50/20 py-3 pl-10 pr-4 text-xs font-mono font-bold text-amber-800 outline-none focus:border-amber-500"
                    />
                  </div>
                </div>
              )}
